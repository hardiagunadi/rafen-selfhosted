        ...
        <div class="card-body">
            ...
            <form method="GET" class="mb-3">
                <div class="form-row align-items-end">
                    <div class="form-group col-md-5">
                        <label for="search">Cari User Hotspot</label>
            ...
            <div class="table-responsive">
                <table class="table table-striped table-hover mb-0">
                    ...
                    <tbody>
                        @forelse($hotspotUsers as $hotspotUser)
                            <tr>
                                <td><input type="checkbox" name="ids[]" value="{{ $hotspotUser->id }}" form="bulk-delete-form"></td>
                                <td>
                                    <a href="#" class="toggle-status-btn badge badge-{{ $hotspotUser->status_akun === 'enable' ? 'success' : 'danger' }}" data-toggle-url="{{ route('super-admin.settings.hotspot-users.toggle-status', $hotspotUser) }}">
                                        {{ $hotspotUser->customer_id ?? '-' }}
                                    </a>
                                </td>
                                <td>
                                    <a href="{{ route('super-admin.settings.hotspot-users.edit', $hotspotUser) }}" class="font-weight-bold text-dark">{{ $hotspotUser->customer_name }}</a>
                                </td>
                                <td>{{ $hotspotUser->username ?? '-' }}</td>
                                <td>{{ $hotspotUser->hotspotProfile?->name ?? '-' }}</td>
                                <td>{{ $hotspotUser->jatuh_tempo?->format('Y-m-d') ?? '-' }}</td>
                                <td>
                                    <span class="badge badge-{{ $hotspotUser->status_akun === 'enable' ? 'success' : ($hotspotUser->status_akun === 'isolir' ? 'warning' : 'secondary') }}">
                                        {{ strtoupper($hotspotUser->status_akun ?? '-') }}
                                    </span>
                                </td>
                                <td class="text-right">
                                    <div class="btn-group btn-group-sm">
                                        <a href="{{ route('super-admin.settings.hotspot-users.edit', $hotspotUser) }}" class="btn btn-warning text-white" title="Edit">
                                            <i class="fas fa-pen"></i>
                                        </a>
                                        <form action="{{ route('super-admin.settings.hotspot-users.destroy', $hotspotUser) }}" method="POST" onsubmit="return confirm('Hapus user hotspot ini?')">
                                            @csrf
                                            @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="8" class="text-center text-muted py-4">Belum ada user Hotspot.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
